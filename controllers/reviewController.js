...
export const createReview = async (req,res) => {
    try {
        ...
        if(bookingCheck.rowCount === 0) {
            return res.status(400).json({message: "Cannot Review, Booking Not Confirmed"})
        }
        ...
        VALUES ($1, $2, $3, $4)
        RETURNING *`
        const response = await pool.query(insertSql, [user_id, booking_id, rating, comment])

        res.status(200).json({message: "Review Created Successfully", data: response.rows[0]})
    } catch (error) {
...
export const getReviewByUserId = async (req,res) => {
    try {
        ...
        res.status(200).json({message: "Get user's reviews successfully", data: response.rows})
    } catch (error) {
...
export const updateReviewById = async (req,res) => {
    try {
        ...
        res.status(200).json({message: "Review updated successfully", data: response.rows[0]})
    } catch (error) {
